...
/**
 * Fetch the PDF through axios (which sends cookies via withCredentials)
 * and return an object-URL blob so the viewer never needs auth itself.
 * This solves the cross-domain cookie problem: the PDF viewer's internal
 * fetch can't attach cookies across origins, but axios can.
 */
function usePdfBlobUrl(rawUrl) {
  ...
  useEffect(() => {
    ...
    axios
      .get(rawUrl, { responseType: 'blob', withCredentials: true })
      .then((res) => {
...
export default function ReaderPage({ source = 'public' }) {
  ...
  useEffect(() => () => { dispatch(resetReader()); }, []); // eslint-disable-line

  // Raw serve URL (absolute, pointing to Railway)
  const rawPdfUrl = source === 'public'
    ? libraryApi.getPdfUrl(id)
    : personalApi.getPdfUrl(id);

  // Fetch via axios so auth cookie is sent, then hand a blob:// URL to the viewer
  const { blobUrl, error: pdfError } = usePdfBlobUrl(rawPdfUrl);
  ...
  return (
    <div className="h-screen flex flex-col bg-parchment-100 dark:bg-ink-900 overflow-hidden">
      ...
      {/* ── Main viewer area ────────────────────────────────────────────── */}
      <div className="flex-1 overflow-hidden relative" onMouseUp={handleMouseUp}>
        {/* Loading state — waiting for blob URL */}
        {!blobUrl && !pdfError && (
          <div className="flex flex-col items-center justify-center h-full gap-4">
            <Spinner size="lg" />
        ...
        {pdfError && (
          <div className="flex flex-col items-center justify-center h-full gap-3 text-center px-4">
            <p className="text-lg font-serif text-ink-700 dark:text-parchment-200">Could not load the PDF</p>
            <p className="text-sm text-ink-400">{pdfError}</p>
            <button onClick={() => navigate(-1)} className="btn-secondary mt-2">Go back</button>
          </div>
        )}

        {/* PDF viewer — only render once blob URL is ready */}
        {blobUrl && (
          <Worker workerUrl={new URL('pdfjs-dist/build/pdf.worker.min.js', import.meta.url).toString()}>
            <div className="h-full">
              <Viewer
                ...
                renderLoader={(pct) => (
                  <div className="flex flex-col items-center justify-center h-full gap-4">
                    ...
                    <p className="text-sm text-ink-400">Rendering… {Math.round(pct)}%</p>
                  </div>
                )}
                renderError={(err) => (
                  <div className="flex flex-col items-center justify-center h-full gap-3 text-center px-4">
                    <p className="text-lg font-serif text-ink-700 dark:text-parchment-200">Could not display the PDF</p>
                    <p className="text-sm text-ink-400">{err?.message || 'The file may be corrupted.'}</p>
                    <button onClick={() => navigate(-1)} className="btn-secondary mt-2">Go back</button>
                  </div>
                )}
              />
